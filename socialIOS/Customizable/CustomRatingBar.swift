    ...
    //
    //  Created by Yash Jain on 28/02/23.
    //

import UIKit

@objc public protocol CustomRatingBarDelegate {
    /// Returns the rating value when touch events end
    @objc optional func customRatingBar(_ ratingView: CustomRatingBar, didUpdate rating: Double)

    /// Returns the rating value as the user pans
    @objc optional func customRatingBar(_ ratingView: CustomRatingBar, isUpdating rating: Double)
}

/// A simple rating view that can set whole, half or floating point ratings.
@IBDesignable
@objcMembers
open class CustomRatingBar: UIView {
    
    // MARK: Properties
    
    open weak var delegate: CustomRatingBarDelegate?
    ...
    private func updateLocation(_ touch: UITouch) {
        ...
        // Update delegate
        delegate?.customRatingBar?(self, isUpdating: rating)
    }
    ...
    override open func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        // Update delegate
        delegate?.customRatingBar?(self, didUpdate: rating)
    }
